...
class Firewall (EventMixin):
	...
	def _handle_PacketIn(self, event):
		...
		if(match.dl_type == packet.ARP_TYPE and match.nw_proto == arp.REQUEST):
		  mymac = match.dl_src
		  myip = match.nw_src
		  if mymac not in addedip:
			  print('add ip')
			  addedip.append(mymac)
			  macippair.append([mymac,myip])
			  print(macippair)
		  self.replyToARP(packet, match, event)


		if(match.dl_type == packet.IP_TYPE):
		  #get ip and mac of packet
		  mymac = match.dl_src
		  myip = match.nw_src
			#check if new mac
		  ...
		  if mymac not in addedip:
			  ...
			  print(macippair)
		  	  ...
		  	  print('verify')
			  for x in macippair: #check if new ip address matches with established IP address
				  
				  if x[0] == mymac:
					  print('foundc')
					  if x[1] != myip: #if established IP is does not match current ip
						  #add mac to ban list
						  print('error')
						  message = of.ofp_flow_mod() # OpenFlow massage. Instructs a switch to install a flow
						  match = of.ofp_match() # Create a match
						  match.dl_src = mymac # Source address
						  match.dl_dst = None # Destination address
						  message.priority = 65535 # Set priority (between 0 and 65535)
						  message.match = match			
